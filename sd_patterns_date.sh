...
search_sd() {
	PATTERN_PRINT=false
	for file in $( rgrep -El -m 1 "$date_range_string" | tr '\n' '\0' | xargs -0 grep -il "$PATTERN" )
	do
		NUMBER_OF_OCCURENCES=$(grep -E "$date_range_string" $file | grep "$PATTERN" | wc -l )
		if [ "$PATTERN_PRINT" = false ] && [ $NUMBER_OF_OCCURENCES -gt 0 ]; then
		...
		if [ $NUMBER_OF_OCCURENCES -gt 0 ]; then
			...
			LAST_LINE=$(grep -E "$date_range_string" $file | grep  "$PATTERN" | tail -1)
			echo -e "        $LAST_LINE"
		fi
	done
...
date_range_string=$(generate_date_range_string "$1" "$2")
