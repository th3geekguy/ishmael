...
import argparse, os, json, fnmatch, numpy, re, zipfile
...
def getddcver(archive, nodename, f_glob, k):
    f = [a for a in archive.namelist() if a.startswith(nodename) and f_glob in a]
    if f == None or f == []:
        return '-'
    else:
        file_content = archive.read(f[0])
...
def full_os_details_sep(archive, hostname):
...
def display_nodes(args, f):
    with zipfile.ZipFile(args.filename, 'r') as archive:
        file_content = archive.read(f)
        sd = json.loads(file_content)
        ...
        for node in sd:
            ...
            os_version, release, hypervisor, kernel = full_os_details_sep(archive, hostname) if os == 'linux' else ('N/A', '-', 'N/A', '-')
            os = os.replace('linux', '🐧')
            ...
            '''Other'''
            cid = node['ID'][:10]
            ucpver = getddcver(archive, hostname,'ucp-proxy.txt','IMAGE_VERSION')
            dtrver = getddcver(archive, hostname,'dtr-registry-','DTR_VERSION')
